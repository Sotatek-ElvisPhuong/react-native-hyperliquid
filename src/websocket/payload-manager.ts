...
export class WebSocketPayloadManager {
  ...
  async executeMethod(
    ...
    try {
      ...
      const response = await this.subscriptions.postRequest(
        'action',
        payload,
        timeout
      );
  ...
  async vaultTransfer(
    ...
    return this.executeMethod('vaultTransfer', {
      vaultAddress,
      isDeposit,
      usd,
    });
  ...
  async tokenDelegate(
    ...
    return this.executeMethod('tokenDelegate', {
      wei,
      validator,
      isUndelegate,
    });
  ...
  async marketOpen(
    ...
    return this.customOperations.marketOpen(
      symbol,
      isBuy,
      size,
      px,
