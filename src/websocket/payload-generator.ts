...
export interface PayloadMethodConfig {
  ...
  payloadTransformer?: (
    params: any,
    context: PayloadGenerationContext
  ) => any | Promise<any>;
...
export const EXCHANGE_METHOD_CONFIGS: Record<string, PayloadMethodConfig> = {
  ...
  transferBetweenSpotAndPerp: {
    ...
    payloadTransformer: (params, context) => ({
      ...
      signatureChainId: context.isMainnet
  ...
  approveBuilderFee: {
    ...
    payloadTransformer: (params, context) => ({
      ...
      signatureChainId: context.isMainnet
        ? CHAIN_IDS.ARBITRUM_MAINNET
        : CHAIN_IDS.ARBITRUM_TESTNET,
      maxFeeRate: params.maxFeeRate.endsWith('%')
        ? params.maxFeeRate
        : `${params.maxFeeRate}%`,
...
export class PayloadGenerator {
  ...
  async generatePayload(methodName: string, params: any): Promise<any> {
    ...
    try {
      ...
      const vaultAddress = config.requiresVaultAddress
        ? this.context.getVaultAddress()
        : null;
      ...
      switch (config.signingMethod) {
        ...
        case 'userSignedAction':
          if (!config.signatureTypes || !config.primaryType) {
            throw new Error(
              `Missing signature configuration for method: ${methodName}`
            );
          }
      ...
      throw new Error(
        `Failed to generate payload for ${methodName}: ${errorMessage}`
      );
    }
  }
