...
import { CHAIN_IDS, ENDPOINTS, ExchangeType } from '../types/constants';
...
export class ExchangeAPI {
  ...
  async placeOrder(
    orderRequest: OrderRequest | Order | BulkOrderRequest
  ): Promise<any> {
    ...
    const isBulkOrder =
      'orders' in orderRequest && Array.isArray(orderRequest.orders);
    ...
    try {
      ...
      const normalizedOrders = ordersArray.map((order: OrderRequest) => {
        ...
        if (typeof normalizedOrder.limit_px === 'string') {
          normalizedOrder.limit_px = removeTrailingZeros(
            normalizedOrder.limit_px
          );
        }
  ...
  async batchModifyOrders(
    modifies: Array<{ oid: number | string; order: Order }>
  ): Promise<any> {
    await this.parent.ensureInitialized();
    try {
      const vaultAddress = this.getVaultAddress();
      ...
      const action = {
        ...
        modifies: normalizedModifies.map((m, index) => {
          ...
          if (assetIndex === undefined) {
            throw new Error(`Asset index not found for coin: ${m.order.coin}`);
          }
          return {
            oid: m.oid,
            order: orderToWire(m.order, assetIndex),
          };
        }),
      };
  ...
  async cancelTwapOrder(
    cancelRequest: TwapCancelRequest
  ...
  async reserveRequestWeight(
    weight: number
  ): Promise<ReserveRequestWeightResponse> {
    await this.parent.ensureInitialized();
    try {
      const vaultAddress = this.getVaultAddress();
      const action = {
