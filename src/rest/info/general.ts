...
export class GeneralInfoAPI {
  ...
  async getAllMids(rawResponse: boolean = false): Promise<AllMids> {
    const response = await this.httpApi.makeRequest<AllMids>({
  ...
  async getMaxBuilderFee(
    ...
    return rawResponse
      ? response
      : this.symbolConversion.convertToNumber(response);
  }
  ...
  async getUserOrderHistory(
    ...
    const response = await this.httpApi.makeRequest<UserOrderHistory>(
      params,
      20
    );
    return rawResponse
